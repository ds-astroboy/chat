...
const WalletSignupButtonGroup: FC<IProps> = (props) => {
    ...
    const signInViaAptosWallet = async() => {
        ...
        console.log("LALALA   ", await wallet.signMessage({message: data.message, nonce: "random_string",}));
        try {
            // await aptosWallet.signMessage(data);
            signature = base58.encode(await wallet.signMessage({message: data.message, nonce: "random_string",}));
        }
        catch(e) {
            console.error(e);
            setIsSigning(false);
