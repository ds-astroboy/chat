...
const WalletSignupButtonGroup: FC<IProps> = (props) => {
    ...
    const signInViaAptosWallet = async() => {
        ...
        console.log("Public Key = ", bs58.encode(wallet.account.publicKey));
        const {success, data, responseCode} = await verifySolanaWallet(bs58.encode(wallet.account.publicKey));
        ...
        const {success: signinResult, data: accountData} = await signInAptosWallet(bs58.encode(wallet.account.publicKey), signature, data.token);
        if(!signinResult)  {
            setIsSigning(false);
            wallet.disconnect();
            return;
        }
        window.localStorage.setItem("conneted_wallet", bs58.encode(wallet.account.publicKey));
        let primaryWallet = {
            protocol: BLOCKCHAINNETWORKS.Aptos,
            address: bs58.encode(wallet.account.publicKey)
        }
        window.localStorage.setItem("primary_wallet", JSON.stringify(primaryWallet));
        let userData = {
            username: accountData.username,
