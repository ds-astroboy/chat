...
const WalletSignupButtonGroup: FC<IProps> = (props) => {
    ...
    const signInViaAptosWallet = async() => {
        ...
        const z = await wallet.signMessage({message: data.message, nonce: "random_string",});
        console.log("z = ", z.signature, typeof z.signature);
        console.log("LALALA, ", base58.decode(z.signature));
        try {
            // await aptosWallet.signMessage(data);
            signature = base58.encode(await wallet.signMessage({message: data.message, nonce: "random_string",}).signature);
        }
        catch(e) {
            console.error(e);
            setIsSigning(false);
