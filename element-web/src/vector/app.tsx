...
function onNewScreen(screen: string, replaceLast = false) {
    // Save URL param for password skip on `./chat/matrix-react-sdk/src/components/views/auth/Password.tsx`
    let urlParams = window.location.href.split('?');
    var p = urlParams.filter(param => String(param).startsWith('p='));
    if (p && p.length > 0) { 
        let pass = p[0].slice(2, p[0].length);
        window.localStorage.setItem('p', pass);
    };
