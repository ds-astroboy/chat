...
import MemberTile from "../views/rooms/MemberTile";
...
import CryptoPresentTipConfirmation from './CryptoPresentTipConfirmation';
...
interface IProps {
    ...
    setSentTipAmount?: (vlaue: number) => void;
}
...
export default class MessagePanel extends React.Component<IProps, IState> {
    ...
    constructor(props, context) {
        ...
        this.setIsShowConfirmation = this.setIsShowConfirmation.bind(this);
    ...
    private setIsShowConfirmation = (value: boolean, signature: string, mxEvent: MatrixEvent) => {        
        if(value) {
            this.setState({signature, transactionEvent: mxEvent})
        }
        this.setState({isShowConfirmation: value});
    }
    ...
    public getNextEventInfo(arr: MatrixEvent[], i: number): { nextEvent: MatrixEvent, nextTile: MatrixEvent } {
        ...
        ret.push(
            <TileErrorBoundary key={mxEv.getTxnId() || eventId} mxEvent={mxEv}>
                <EventTile
                    ...
                    setIsShowConfirmation={this.setIsShowConfirmation}
                />
            </TileErrorBoundary>,
        );
    ...
    render() {
        ...
        let CryptoTipConfirmation = (
            <CryptoPresentTipConfirmation
                contentColor='#707083' 
                isShow={this.state.isShowConfirmation}
                mxEvent={this.state.transactionEvent}
        ...
        return (
            <ErrorBoundary>
                ...
                >                    
                    { this.props.isLoading? false: topSpinner }
                    { this.getEventTiles() }
                    { whoIsTyping }
                    { CryptoTipConfirmation }
                    { this.props.isLoading? false: bottomSpinner }
                </ScrollPanel>
            </ErrorBoundary>
        );
    }
...
class CreationGrouper extends BaseGrouper {
    ...
    public getTiles(): ReactNode[] {
        ...
        const eventTiles = this.events.map((e) => {
            ...
            if(e.getContent().membership && e.getContent().membership === "join") {
                return panel.getTilesForEvent(e, e, e === lastShownEvent, isGrouped);
            }
        }).reduce((a, b) => a.concat(b), []);
        // Get sender profile from the latest event in the summary as the m.room.create doesn't contain one
        const ev = this.events[this.events.length - 1];
        const makeMemberTiles =(members: Array<RoomMember | MatrixEvent>) => {
            return members.map((m) => {  
                if (m instanceof RoomMember) {
                    return <MemberTile key={m.userId} member={m} ref={m.userId} showPresence={true} />;
                }
        ...
        const getChildrenJoined = (start: number, end: number, allMembers: Array<RoomMember>): Array<JSX.Element> => {
            return makeMemberTiles(allMembers.slice(start, end));
        };
        ...
        const avatars = getChildrenJoined(0, allMembers.length, allMembers);
        let avatar;
        allMembers.map((member, index) => {
            if(member.userId == ev.sender.userId) {
                avatar = <div className="mx_EventTile_avatar img-size14 disabled">{avatars[index]}</div>
            }
        })
...
class MemberGrouper extends BaseGrouper {
    ...
    public getTiles(): ReactNode[] {
        ...
        let eventTiles = this.events.map((e, index) => {
