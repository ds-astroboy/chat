...
import BarrierContextProvider from './contextProvider/BarrierContextProvider';
import { RightPanelPhases } from '../../stores/RightPanelStorePhases';
import { SetRightPanelPhasePayload } from '../../dispatcher/payloads/SetRightPanelPhasePayload';
import { CurrentBackgroundProvider } from './contextProvider/BackgroundProvider';
import { checkBarrierRooms } from '../../apis';
...
export interface IState {
    ...
    roomBarrierInfo?: any;
    isLoadingRoombarrier?: boolean;
    isShowMessageTap: boolean;
}
...
export default class RoomView extends React.Component<IProps, IState> {
    ...
    constructor(props, context) {
        ...
        this.state = {
            ...
            lottieBgUrl: null,
            sentTipAmount: 0,
            roomBarrierInfo: null,
            isLoadingRoombarrier: false,
            isShowMessageTap: true,
        };
        ...
        this.setSentTipAmount = this.setSentTipAmount.bind(this);
        this.setRoomBarrierInfo = this.setRoomBarrierInfo.bind(this);
    }
    ...
    componentDidMount() {
        ...
        const otherUserId = DMRoomMap.shared().getUserIdForRoomId(this.props.roomId);
        if (otherUserId && room.getJoinedMemberCount() === 2) {
            const rigntPanel = JSON.parse(localStorage.getItem("mx_lastRightPanelPhaseForRoom"))?.value;
            const isShow = JSON.parse(localStorage.getItem("mx_showRightPanelInRoom"))?.value;
            if (isShow && rigntPanel && rigntPanel !== RightPanelPhases.RoomSummary) {
    ...
    private setSentTipAmount = (sentTipAmount) => {
        this.setState({ sentTipAmount })
    }

    private setRoomBarrierInfo = async () => {
        this.setState({ isLoadingRoombarrier: true })
        let roomBarrierInfo = await checkBarrierRooms(this.props.roomId);
        this.setState({ roomBarrierInfo, isLoadingRoombarrier: false });
    }
    ...
    private onSearchClick = () => {
        this.setState({
            isThisRoomSearching: !this.state.isThisRoomSearching
        });
    };
    ...
    setLottieBgUrl = (url) => {
        this.setState({ lottieBgUrl: url });
    }

    handleMessageTap = () => {
        this.setState({ isShowMessageTap: !this.state.isShowMessageTap })
    }
    ...
    render() {
                    ...
                    <div className="mx_RoomView">
                        <ErrorBoundary>
                            <RoomPreviewBar
                                ...
                                isSetCurrencyBarrier={true}
                                barrierInfo={this.state.roomBarrierInfo}
                                roomId={this.props.roomId}
                                isLoadingRoombarrier={this.state.isLoadingRoombarrier}
                            />
                        </ErrorBoundary>
                    </div>
                );
        ...
        let aux = null;
        let previewBar;
        if (this.state.isThisRoomSearching) {
            aux = <SearchBar
        ...
        if (canSpeak) {
            messageComposer =
                <MessageComposer
                    ...
                    receivedTipAmount={this.props.receivedTipAmount}
                    setReceivedTipAmount={this.props.setReceivedTipAmount}
                    sentTipAmount={this.state.sentTipAmount}
                    setSentTipAmount={this.setSentTipAmount}
                    isMobileView={this.props.isMobileView}
                    isHiddenLeftPanel={this.props.isHiddenLeftPanel}
                />;
        }
        ...
        const rightPanel = showRightPanel
            ? <RightPanel room={this.state.room} resizeNotifier={this.props.resizeNotifier} setLottieBgUrl={this.setLottieBgUrl} setSentTipAmount={this.setSentTipAmount} />
            : null;
        ...
        const mainClasses = classNames("mx_RoomView", {
            ...
            mx_RoomView_Mobile: this.props.isMobileView,
        });
                ...
                <main className={mainClasses} ref={this.roomView} onKeyDown={this.onReactKeyDown}>
                    ...
                    <div className="mx_RoomView_container">
                        <ErrorBoundary>
                            ...
                            <MainSplit roomId={this.state.room.roomId} panel={rightPanel} resizeNotifier={this.props.resizeNotifier} lottieBgUrl={this.state.lottieBgUrl}>
                                <div className="mx_RoomView_body">
                                    {auxPanel}
                                    <div className={timelineClasses} id="mx_RoomView_timeline">
                                        <CurrentBackgroundProvider room={this.state.room} setLottieBgUrl={this.setLottieBgUrl}>
                                            {fileDropTarget}
                                            {topUnreadMessagesBar}
                                            {jumpToBottom}
                                            {messagePanel}
