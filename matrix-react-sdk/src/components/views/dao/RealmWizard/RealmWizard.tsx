...
const RealmWizard: React.FC<IProps> = ({ onFinished }) => {
  ...
  const handleCreateMultisigRealm = async () => {
    ...
    if (results) {
        dis.dispatch({
            ...
            symbol: results.realmPk.toBase58()
        })
        onFinished(false);
  ...
  return (
    <div
        className="mx_RealmWizard"
    >
        <AccessibleButton 
            className="mx_RealmWizard_backButton"
            onClick={handleBackButtonClick}
        >
            <ArrowLeftIcon className="mx_RealmWizard_backButton_icon" />
            <div className='mx_RealmWizard_backButton_content'>
                Back
            </div>
        </AccessibleButton>
        {isLoading ? (
            <div className="mx_RealmWizard_loading">
                <Spinner />
                <div className='mx_RealmWizard_loading_content'>{loaderMessage}</div>
            </div>
        ) : (
            <div className="mx_RealmWizard_body">{BoundStepComponent}</div>
        )}
        {ctl && !(ctl.isModeSelect() || isLoading) && (
            <>
            <div
                className={`mx_RealmWizard_buttonGroup`}
            >
                {!ctl.isFirstStep() ? (
                <AccessibleButton
                    onClick={() => {
                    ...
                    className="mx_RealmWizard_button"
                >
                    Previous
                </AccessibleButton>
                ) : (
