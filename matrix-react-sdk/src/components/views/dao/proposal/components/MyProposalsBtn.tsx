...
import { sendSignedTransaction } from '../../../../../utils/vote/sendTransactions'
...
const MyProposalsBn = () => {
  ...
  return (
    <>
      <div>
        <AccessibleButton onClick={() => setModalIsOpen(true)}>My proposals</AccessibleButton>
      </div>
      {modalIsOpen && (
        <div
        ...
        >
          <>
            <h3 className="mb-4 flex flex-col">
              Your proposals {isLoading && <Spinner w={50}></Spinner>}
            </h3>
            <ProposalList
              title="Drafts"
              fcn={cleanDrafts}
      ...
      <h4 className="flex items-center mb-3">
        ...
        {btnName && proposals.length !== 0 && (
          <div className="ml-auto">
            ...
            <AccessibleButton onClick={() => fcn()} disabled={isLoading}>
              {btnName} all
            </AccessibleButton>
          </div>
        )}
