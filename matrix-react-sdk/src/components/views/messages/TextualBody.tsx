...
export default class TextualBody extends React.Component<IBodyProps, IState> {
    ...
    UNSAFE_componentWillReceiveProps(nextProps: Readonly<IBodyProps>): void {
    ...
    private linkCheck (content: string) {
        let previewUrls = [];
        let expression = /(https?:\/\/)?[\w\-~]+(\.[\w\-~]+)+(\/[\w\-~@:%]*)*(#[\w\-]*)?(\?[^\s]*)?/gi;
        content.replace(expression, (url) => {
            let result = false;
            let previewlink = { url: "", tool: "MicroLink" };
            if(content.search(":main.cafeteria.gg") == -1) {
                if(url.search("http") == -1) {
                    previewlink.url = "https://" + url;
                }
                else {
                    previewlink.url = url;
                }
                if(url.search("imgur.com") != -1) {
                    previewlink.tool = "LinkPreview";
                }
                previewUrls.push(previewlink);
                return url
            }
        })
        return previewUrls;
    ...
    private customFetcher = async (url: string) => {
        const response = await fetch(`https://fetcher.cafeteria.gg/fetcherAPI/?url=${url}`);
        const json = await response.json();

        return json.metadata;
    }

    private youtubeURLParser = (url: string): string|null => {
    ...
    render() {
        ...
        const previewUrls = this.linkCheck(content.body);
        ...
        let widgets;
        if (previewUrls.length && !this.state.widgetHidden && this.props.showUrlPreview) {
        ...
        // console.log("====this.props.giftsRow===", this.props.giftsRow);

        switch (content.msgtype) {
            case MsgType.Emote:
                return (
                    <span className="mx_MEmoteBody mx_EventTile_content"  onContextMenu={this.props.showMessageActionBar}>
                        *&nbsp;
            ...
            case MsgType.Notice:
                return (
                    <span className="mx_MNoticeBody mx_EventTile_content"  onContextMenu={this.props.showMessageActionBar}>
                        { this.props.actionBar }
                        { body }
                        { widgets }
                    </span>
            ...
            default: // including "m.text"
                return (
                    <>
                        <span className="mx_MTextBody mx_EventTile_content" onContextMenu={this.props.showMessageActionBar}>
                            { this.props.actionBar }
                                <div className='mx_MTextBody_container' style={{borderRadius: previewUrls.length ? '10px 10px 0 0' : '10px'}}>
                                { this.props.giftsRow? this.props.giftsRow: null }
                                    { this.props.senderAndGift? this.props.senderAndGift : null }
                                    { body }
                                    <br/>
                                    { widgets }
                            </div>
                        </span>
                        <div className='mx_LinkPreview_body'>
                            {
                                previewUrls?
                                    previewUrls.map((previewUrl) => {
                                        if ((previewUrl.url).includes('twitter.com')){
                                            const tweetId = (previewUrl.url).split('/').slice(-1)[0].split('?')[0];
                                            return <TwitterTweetEmbed
                                                    tweetId={tweetId}
                                                />
                                        } else if ((previewUrl.url).includes('youtube.com') || ((previewUrl.url).includes('youtu.be'))){
                                            const videoId = this.youtubeURLParser(previewUrl.url);
                                            const opts = {
                                                height: '315',
                                                width: '560',
                                            };
                                            return <YouTube videoId={videoId} opts={opts} iframeClassName="youtube-embed-iframe" />
                                        } else {
                                            return <LinkPreview borderRadius='0px 10px 10px 10px' width="500px" height="250px" fetcher={this.customFetcher} showLoader={true} url={previewUrl.url} />
                                        }
                                    })
                                :
                                false
