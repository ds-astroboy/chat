...
import BarrierCheckContainer from "./BarrierCheckContainer"
import BarrierContextProvider from "../../structures/contextProvider/BarrierContextProvider"
...
export default class RoomPreviewBar extends React.Component {
    static propTypes = {
        ...
        isLoadingRoombarrier: PropTypes.bool,
    };
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        let roomId = this.props.room ? this.props.room.roomId : this.props.roomId;

        if(!this.props.barrierInfo && !this.state.busy && roomId && !this.props.isLoadingRoombarrier) {
    ...
    render() {
        ...
        if(this.props.isLoadingRoombarrier) {
            roomPreviewContainer = 
            <h3><Spinner/> Checking Room Barrier...</h3>
        }
        ...
        if(this.props.barrierInfo && (!room || !isJoined)) {
            roomPreviewContainer = (
                <BarrierCheckContainer 
                    roomId={roomId} 
                    joinClick={this.props.onJoinClick} 
                    barrierInfo={this.props.barrierInfo}
                />
            )
        }       

        if(messageCase === MessageCase.RoomNotFound || messageCase === MessageCase.OtherError) {
            roomPreviewContainer = (
                <div className='mx_RoomPreviewBar_error'>
                    <div className='mx_RoomPreviewBar_error_img'>
                        <img src={require("../../../../res/img/cafeteria-errorscreen.svg")}/>
                    </div>
                    <div className='mx_RoomPreviewBar_error_header'>Oops!</div>
                    <div className='mx_RoomPreviewBar_error_content'>The page you are looking for does not exist.</div>
                </div>
            )
        }
