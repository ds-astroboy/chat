...
export default class MemberList extends React.Component<IProps, IState> {
    ...
    constructor(props) {
        ...
        // this.getUserPoints(cli.getUserId());
    }
    ...
    private listenForMembersChanges(): void {
        console.log("----------listenForMembersChanges------------")
        const cli = MatrixClientPeg.get();
    ...
    // private getUserPoints = async(userId: string) => {
    //     let userPoints;
    //     let accessToken = await this.getAccessToken();
    //     if(accessToken) {
    //         await axios.get(
    //             `https://stage.cafeteria.gg/dashboard/api/total_user_points/${userId}/`,
    //             {
    //                 headers: {
    ...
    private makeMemberTiles(members: Array<RoomMember | MatrixEvent>) {
        return members.map((m) => {
            if (m instanceof RoomMember) {
                ...
                if(m.user.presence == "online" || m.userId == userId) {
                    return (
                        <NftAvatarCheckContainer userId={m.userId}>
                            <MemberTile key={m.userId} member={m} ref={m.userId} showPresence={this.showPresence} />
    ...
    private getChildrenJoined = (start: number, end: number): Array<JSX.Element> => {
        ...
        return this.makeMemberTiles(this.state.filteredJoinedMembers);
    };
    ...
    render() {
        ...
        const cli = MatrixClientPeg.get();
        const room = cli.getRoom(this.props.roomId);
        const isPrivate = !!room.summaryHeroes;
        let inviteButton;
        let userPoints;
        if(this.state.userPoints >= 1000) {
            userPoints = this.state.userPoints / 1000;
        }
        else userPoints = this.state.userPoints
        ...
        const showDonatorPanel = () => {
            dis.dispatch<SetRightPanelPhasePayload>({
                action: Action.SetRightPanelPhase,
                phase: RightPanelPhases.RoomDonatorList,
            });
        }
        
        let userId = MatrixClientPeg.get().getUserId();
        let user = MatrixClientPeg.get().getUser(userId);
        const displayNameClassName = classNames("mx_RightPanel_userInfo_name")
        // const isVerified = checkVerifiedUserOrRoom(userId, user.displayName);
        // let verifiedBadge;
        // if(isVerified) {
            ...
            <div className='mx_RightPanel_userInfo'>
                <div className='mx_RightPanel_userInfo_section'>
                    <NftAvatarCheckContainer userId={userId}>
                        <BaseAvatar
                            url={user.avatarUrl ? mediaFromMxc(user.avatarUrl).getSquareThumbnailHttp(40) : null}
                    ...
                    <div className='mx_RightPanel_userInfo_detail'>
                        <VerifiedCheckContainer isUser={true} id={userId} className={displayNameClassName} roomId={this.props.roomId}>
                        ...
                        {/* <div className='mx_RightPanel_userInfo_point'>
                            ...
                            <span>{this.state.userPoints} Credits</span>
                        </div>                     */}
                    </div>
                </div>
                {/* <div className='mx_RightPanel_userInfo_moreButton'>
                    <button onClick={showDonatorPanel}></button>
                </div> */}
            </div>
        )
        ...
        return <BaseCard
            ...
            header={<React.Fragment>                
                ...
                {/* { userInfo } */}
                {/* {
                    isPrivate || !this.state.canInvite?
                    false
                    :
                    inviteButton
                } */}
        ...
        >
            <div className="mx_MemberList_wrapper">
                <div className='mx_MemberList_title bold grey my-2'>Online</div>
                <OnlineMemberList
