...
import OnlineMembersNumber from "./OnlineMembersNumber";
...
export default class RoomTile extends React.PureComponent<IProps, IState> {
    ...
    private onRejectClick = (ev: ButtonEvent) => {
        ev.preventDefault();
        ev.stopPropagation();
        this.setState({isRejecting: true});
        MatrixClientPeg.get().leave(this.props.room.roomId).then(() => {
    ...
    private renderGeneralMenu(isCreator: boolean): React.ReactElement {
        ...
        if (this.state.generalMenuPosition && this.props.tag === DefaultTagID.Archived) {
        ...
        else if(this.state.generalMenuPosition && this.props.tag === DefaultTagID.Invite) {
            contextMenu = <IconizedContextMenu
                {...contextMenuBelow(this.state.generalMenuPosition)}
                onFinished={this.onCloseGeneralMenu}
                className="mx_RoomTile_contextMenu"
            ...
            const lowPriorityLabel = "Archive";
            ...
            >
                <IconizedContextMenuOptionList>
                    ...
                    <IconizedContextMenuCheckbox
                        ...
                        iconClassName="mx_RoomTile_iconArchive"
                    />
                    { canInvite ? (
                        <IconizedContextMenuOption
    ...
    public render(): React.ReactElement {
        ...
        let roomCreator;
        const myUserId = this.props.room.myUserId;
        let isCreator = false;
        this.props.room.currentState.events.forEach((eventMap) => {
        ...
        let membersNumber;
        if(this.props.room.getJoinedMemberCount() >= 1000) {
            membersNumber = membersNumber/1000 + "k";
        }
        else {
            membersNumber = this.props.room.getJoinedMemberCount();
        }
        ...
        let joinedMemberCount = this.props.room.getJoinedMemberCount();
        let verifiedBadge;
        const isVerified = checkVerifiedUserOrRoom(this.props.room.roomId, name);
        if(isVerified) {
            verifiedBadge = (
                ...
                <RovingTabIndexWrapper inputRef={this.roomTileRef}>
                    { ({ onFocus, isActive, ref }) =>
                        ...
                        >
                            ...
                            { nameContainer }
                            { badge }
                            { this.renderGeneralMenu(isCreator) }
                            { this.renderNotificationsMenu(isActive) }
                            <div className="mx_RoomMembers_number">
                                <OnlineMembersNumber room={this.props.room}></OnlineMembersNumber>
                            </div>
                            { spinner }
                        </Button>
                    }
