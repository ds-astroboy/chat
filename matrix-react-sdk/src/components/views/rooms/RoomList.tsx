...
import { checkHideRoom } from "../../../hooks/commonFuncs";
...
const WELCOMEPAGE_ALWAYS_VISIBLE_TAGS: TagID[] = [
    DefaultTagID.Recommends,
];
...
const TAG_AESTHETICS: ITagAestheticsMap = {
    ...
    [DefaultTagID.Favourite]: {
        sectionLabel: _td("Favorites"),
        isInvite: false,
        defaultHidden: false,
    },
    [DefaultTagID.DM]: {
        sectionLabel: _td("Private Messages"),
        isInvite: false,
        defaultHidden: false,
    ...
    [DefaultTagID.Recommends]: {
        sectionLabel: "Recommended Groups",
        isInvite: false,
        defaultHidden: false,
    },
    [DefaultTagID.ServerNotice]: {
...
export default class RoomList extends React.PureComponent<IProps, IState> {
    ...
    constructor(props: IProps) {
        ...
        this.state = {
            sublists: {},
            isNameFiltering: this.props.isSignout? false : !!RoomListStore.instance.getFirstNameFilterCondition(),
            suggestedRooms: this.props.isSignout? [] : SpaceStore.instance.suggestedRooms,
            chunkRooms: []
        };
        ...
        cli.publicRooms({})
        .then(data => {
            let chunkRooms:IPublicRoomsChunkRoom[] = [];
            if(data.chunk.length > 0) {
                let count = 0;
                data.chunk.map((item) => {
                    let isHideRoom = checkHideRoom(item.name);
                    if(!cli.getRoom(item.room_id)) {
                        if(count < 4 && !isHideRoom) {
                            chunkRooms.push(item);
                            count ++;
                        }
                    }
                })
                this.setState({chunkRooms});
            }
        })
    }
        ...
        return TAG_ORDER.reduce((tags, tagId) => {
            ...
            .map((orderedTagId, index) => {
                ...
                return <RoomSublist
                    ...
                    label={aesthetics.sectionLabelRaw ? aesthetics.sectionLabelRaw : aesthetics.sectionLabel}
                    ...
                    alwaysVisible={this.props.isSignout? WELCOMEPAGE_ALWAYS_VISIBLE_TAGS.includes(orderedTagId) : ALWAYS_VISIBLE_TAGS.includes(orderedTagId)}
                    onListCollapse={this.props.onListCollapse}
                    chunkRooms={this.state.chunkRooms}
