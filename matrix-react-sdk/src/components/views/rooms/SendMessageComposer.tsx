...
import { priceBotCheck} from '../bots/price_bot';
import { checkBadWords } from "../bots/bad_word_bot"
import Lottie from "lottie-react";
import typingLottie from "../../../../res/img/lottie/cafeteria_animation_nopadding.json";
import classNames from 'classnames';
import { debounce } from "lodash";
import {getRoomHasVerificationEnabled} from '../bots/verification_bot';
import { getUserActivated } from '../bots/verification_bot';
import createRoom from '../../../createRoom';
import UserVerificationDialog from '../dialogs/UserVerificationDialog';
...
export function createMessageContent(
    ...
    const messageIsGIF = isGIF(model);
    if (isEmote) {
    ...
    const content: IContent = {
        msgtype: isEmote ? "m.emote" : (messageIsGIF ? "m.gif" : "m.text"),
        body: body,
    };
...
export default class SendMessageComposer extends React.Component<IProps, IState> {
    ...
    public async sendMessage(): Promise<void> {
        // TODO: Make sure model is not empty, while also allowing GIFs to bypass this validation part.
        ...
        const roomVerificationEnabled = await getRoomHasVerificationEnabled(this.props.room.roomId)
        if (roomVerificationEnabled){
            const userActivated = await getUserActivated(userId, this.props.room.roomId)
            if (!userActivated){
                Modal.createTrackedDialog<[boolean]>(
                    "User Verification",
                    "",
        ...
        if (!containsEmote(this.model) && this.isSlashCommand()) {
            ...
            this.model.reset([]);
            if (!cmd) {
                const {isCurrency, currencyInfo} = await priceBotCheck(commandText);
                if(isCurrency) {
                    botName = "Reon - Price Bot";
                    const startTime = CountlyAnalytics.getTimestamp();
                    const { roomId } = this.props.room;
                    const prom = this.context.sendMessage(roomId, currencyInfo);
                    if (replyToEvent) {
                        // Clear reply_to_event as we put the message into the queue
                        // if the send fails, retry will handle resending.
                        dis.dispatch({
                    ...
                    dis.dispatch({ action: "message_sent" });
                    CHAT_EFFECTS.forEach((effect) => {
                        if (containsEmoji(currencyInfo, effect.emojis)) {
                            dis.dispatch({ action: `effects.${effect.command}` });
                        }
                    });
                    CountlyAnalytics.instance.trackSendMessage(startTime, prom, roomId, false, !!replyToEvent, currencyInfo);
                    // clear composer
                    this.clearStoredEditorState();
                    if (SettingsStore.getValue("scrollToBottomOnMessageSent")) {
                        dis.dispatch({ action: "scroll_to_bottom" });
    ...
    public onAction = (payload: ActionPayload): void => {
    ...
    private getUserTypingEvent = debounce(() => {
    ...
    render() {
        ...
        if(this.state.isUserTyping) {
            typingAvatar = <div>
                <Lottie animationData={typingLottie} className='User_Typing_Lottie'/>
            </div>
        }
        let className = classNames(
            "mx_SendMessageComposer",
            {
                user_no_typing: !this.state.isUserTyping
            }
        )
        return (
            <div className={className} onClick={this.focusComposer} onKeyDown={this.onKeyDown}>
                {typingAvatar}
