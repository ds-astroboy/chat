...
import {
  ListAlgorithm,
  SortAlgorithm,
} from "../../../stores/room-list/algorithms/models";
...
interface IProps {
  ...
  chunkRooms: IPublicRoomsChunkRoom[];
...
interface IState {
  ...
  isShowRecommendsRoomTiles: boolean;
  isBarrierRooms: boolean[];
}
...
export default class RoomSublist extends React.Component<IProps, IState> {
  ...
  constructor(props: IProps) {
    ...
    this.layout = this.props.isSignout
      ? null
      : RoomListLayoutStore.instance.getLayoutFor(this.props.tagId);
    this.heightAtStart = 0;
    this.isBeingFiltered = this.props.isSignout
      ? null
      : !!RoomListStore.instance.getFirstNameFilterCondition();
    this.notificationState = this.props.isSignout
      ? null
      : RoomNotificationStateStore.instance.getListState(this.props.tagId);
    this.state = {
      contextMenuPosition: null,
      addRoomContextMenuPosition: null,
      isResizing: false,
      isExpanded: this.props.isSignout
        ? false
        : this.isBeingFiltered
        ? this.isBeingFiltered
        : !this.layout.isCollapsed,
      height: 0, // to be fixed in a moment, we need `rooms` to calculate this.
      rooms: this.props.isSignout
        ? []
        : arrayFastClone(
            RoomListStore.instance.orderedLists[this.props.tagId] || []
          ),
      isShowRecommendsRoomTiles: true,
      isBarrierRooms: [],
    };
  ...
  private calculateInitialHeight() {
    const requestedVisibleTiles = Math.max(
      Math.floor(this.layout.visibleTiles),
      this.layout.minVisibleTiles
    );
  ...
  public componentDidUpdate(
    ...
    if (
      RoomSublist.calcNumTiles(prevState.rooms, prevExtraTiles) !==
      this.numTiles
    ) {
  ...
  private checkRoom = async (room) => {
    let result = false;
    try {
      await axios
        .get(`${httpsRequest.GetRoomBarrier}/${room.room_id}`)
        .then((response) => {
  ...
  public shouldComponentUpdate(
    ...
    if (this.props.isSignout) return false;
    ...
    const nextExtraTiles =
      nextState.filteredExtraTiles || nextProps.extraTiles || [];
    ...
    if (
      RoomSublist.calcNumTiles(nextState.rooms, nextExtraTiles) !==
      this.numTiles
    ) {
      return true;
    }
  ...
  public componentDidMount() {
    const checkRecommentBarrierRooms = async () => {
      let isBarrierRooms = [];
      for (let i = 0; i < this.props.chunkRooms.length; i++) {
        let isBarrierRoom = await this.checkRoom(this.props.chunkRooms[i]);
        isBarrierRooms.push(isBarrierRoom);
      }
      this.setState({ isBarrierRooms });
    };
    checkRecommentBarrierRooms();
    if (this.props.isSignout) return;
  ...
  private onListsUpdated = () => {
    ...
    if (this.props.extraTiles) {
      const nameCondition =
        RoomListStore.instance.getFirstNameFilterCondition();
      if (nameCondition) {
        stateUpdates.filteredExtraTiles = this.props.extraTiles.filter((t) =>
          nameCondition.matches(normalize(t.props.displayName || ""))
        );
    ...
    if (
      ...
      setImmediate(() => {
        ...
        if (roomIndex >= this.numVisibleTiles) {
          this.layout.visibleTiles = this.layout.tilesWithPadding(
            roomIndex + 1,
            MAX_PADDING_HEIGHT
          );
  ...
  private applyHeightChange(newHeight: number) {
    const heightInTiles = Math.ceil(
      this.layout.pixelsToTiles(newHeight - this.padding)
    );
  ...
  private onShowAllClick = () => {
    ...
    const newHeight = this.layout.tilesToPixelsWithPadding(
      this.numTiles,
      this.padding
    );
    this.applyHeightChange(newHeight);
  ...
  private onShowLessClick = () => {
    const newHeight = this.layout.tilesToPixelsWithPadding(
      this.layout.defaultVisibleTiles,
      this.padding
    );
    this.applyHeightChange(newHeight);
    this.setState({ height: newHeight });
  ...
  private focusRoomTile = (index: number) => {
    ...
    const elements =
      this.sublistRef.current.querySelectorAll<HTMLDivElement>(".mx_RoomTile");
  ...
  private onUnreadFirstChanged = () => {
    const isUnreadFirst =
      RoomListStore.instance.getListOrder(this.props.tagId) ===
      ListAlgorithm.Importance;
    const newAlgorithm = isUnreadFirst
  ...
  private onBadgeClick = (ev: React.MouseEvent) => {
    ...
    if (this.props.tagId === DefaultTagID.Invite) {
      ...
      room = RoomListStore.instance.unfilteredLists[this.props.tagId].find(
        (r: Room) => {
          const notifState = this.notificationState.getForRoom(r);
          return (
            notifState.count > 0 &&
            notifState.color === this.notificationState.color
          );
        }
      );
    }
  ...
  private onHeaderClick = () => {
    if (this.props.tagId == DefaultTagID.Recommends) {
      this.setState({
        isShowRecommendsRoomTiles: !this.state.isShowRecommendsRoomTiles,
      });
    }
    ...
    const isAtBottom =
      listScrollTop >= Math.round(list.scrollHeight - list.offsetHeight);
    const isStickyTop = possibleSticky.classList.contains(
      "mx_RoomSublist_headerContainer_stickyTop"
    );
    const isStickyBottom = possibleSticky.classList.contains(
    ...
    switch (action) {
      ...
      case RoomListAction.ExpandSection: {
        ...
        if (!this.state.isExpanded) {
          ...
          const element = this.sublistRef.current.querySelector(
            ".mx_RoomTile"
          ) as HTMLDivElement;
          if (element) {
            element.focus();
  ...
  private showRecommendRoom = (roomId: string) => {
  ...
  private renderVisibleTiles(): React.ReactElement[] {
    ...
    if (this.props.tagId == DefaultTagID.Recommends) {
      this.props.chunkRooms.map(
        (room: IPublicRoomsChunkRoom, index: number) => {
          let memberNumber;
          if (room.num_joined_members >= 1000) {
            memberNumber = room.num_joined_members / 1000 + "k";
          } else {
            memberNumber = room.num_joined_members;
          }
          const nameClasses = classNames({
            mx_RoomTile_nameContainer: true,
            mx_Minimixed_name: this.props.isMinimized,
          });
          const roomAvatarClassName = classNames({
            mx_RoomAvatar: true,
            mx_minimized_RoomTile: this.props.isMinimized,
            badge_Room: this.state.isBarrierRooms[index],
          });

          const roomTileNameClassName = classNames({
            mx_RoomTile_name: true,
          });
          let roomBadge;
          if (this.state.isBarrierRooms[index]) {
            roomBadge = (
              <div className="mx_RoomAvatar_badge">
                <div className="mx_RoomAvatar_badge_logo barrier_badge"></div>
              </div>
            );
          }
          let cli = MatrixClientPeg.get();
          if (!cli) {
            const hsUrl = "https://main.cafeteria.gg:8448";
          ...
          let avatarUrl = room.avatar_url
            ? mediaFromMxc(room.avatar_url, cli).getThumbnailOfSourceHttp(
                150,
                150,
                "scale"
              )
          ...
          let verifiedBadge;
          if (isVerified) {
            verifiedBadge = <div className="mx_Room_verified"></div>;
          }
          let isHideRoom = checkHideRoom(room.name);
          if (isHideRoom) return;
          tiles.push(
            <Button title={room.name} onClick={null} key={room.room_id}>
              <div
                className={`mx_AccessibleButton mx_RoomTile ${
                  this.props.isMinimized ? "mx_minimized_RoomTile" : ""
                }`}
                role="treeitem"
                aria-label={room.name}
                key={room.name}
                onClick={() => {
                  this.showRecommendRoom(room.room_id);
                }}
              >
                <div className={roomAvatarClassName}>
                  <BaseAvatar
                    url={avatarUrl}
                    name={room.name}
                    width={40}
                ...
                <div
                  ...
                  dir="auto"
                >
                  <div className={roomTileNameClassName}>
                    <span>
                      {room.name}
                    </span>
                    {verifiedBadge}
                ...
                <div className="mx_RoomMembers_number">{memberNumber}</div>
              </div>
            </Button>
          );
        }
  ...
  private renderMenu(): React.ReactElement {
    ...
    if (this.state.contextMenuPosition) {
      const isAlphabetical =
        RoomListStore.instance.getTagSorting(this.props.tagId) ===
      ...
      contextMenu = (
        <IconizedContextMenu
          ...
          top={
            this.state.addRoomContextMenuPosition.top +
            this.state.addRoomContextMenuPosition.height
          }
  ...
  private renderHeader(roomListCategory: string): React.ReactElement {
    return (
      <RovingTabIndexWrapper inputRef={this.headerButton}>
        {({ onFocus, isActive, ref }) => {
          ...
          const badge = this.props.isSignout ? null : (
            <NotificationBadge
              forceCount={true}
          ...
          if (!!this.props.onAddRoom && roomListCategory != "Direct Messages") {
            addRoomButton = (
          ...
          const collapseClasses = classNames({
            ...
            mx_RoomSublist_collapseBtn_collapsed: this.props.tagId == DefaultTagID.Recommends? !this.state.isShowRecommendsRoomTiles : !this.state.isExpanded,
          });
          ...
          const badgeContainer = (
            <div className="mx_RoomSublist_badgeContainer">{badge}</div>
          );
            ...
            >
              <div className="mx_RoomSublist_stickable">
                <Button
                  ...
                  inputRef={ref}
                  tabIndex={tabIndex}
                  className="mx_RoomSublist_headerText bold"
                  role="treeitem"
                ...
                {this.props.isMinimized &&
                this.props.label == "Private Messages" ? (
                  <Button
                    className="mx_RoomSublist_Minimum_Icon"
                    title={this.props.label}
                    onClick={null}
                  >
                    <img
                      src={require("../../../../res/img/leftbar_grop_icons/private_messages.svg")}
                      width={25}
                    />
                  </Button>
                ) : this.props.isMinimized &&
                  this.props.label == "Your Groups" ? (
                  <Button
                    className="mx_RoomSublist_Minimum_Icon"
                    title={this.props.label}
                    onClick={null}
                  >
                    <img
                      src={require("../../../../res/img/leftbar_grop_icons/your_groups.svg")}
                      width={25}
                    />
                  </Button>
                ) : this.props.isMinimized &&
                  this.props.label == "Recommended Groups" ? (
                  <Button
                    className="mx_RoomSublist_Minimum_Icon mx_Recommended_Icon"
                    title={this.props.label}
                    onClick={null}
                  >
                    <img
                      src={require("../../../../res/img/leftbar_grop_icons/recommended_groups.svg")}
                      width={25}
                    />
                  </Button>
                ) : this.props.isMinimized &&
                  this.props.label == "Favorites" ? (
                  <Button
                    className="mx_RoomSublist_Minimum_Icon mx_Recommended_Icon"
                    title={this.props.label}
                    onClick={null}
                  >
                    <img
                      src={require("../../../../res/img/leftbar_grop_icons/favorite.svg")}
                      width={25}
                    />
                  </Button>
                ) : this.props.isMinimized &&
                  this.props.label == "Invites" ? (
                  <Button
                    className="mx_RoomSublist_Minimum_Icon mx_Recommended_Icon"
                    title={this.props.label}
                    onClick={null}
                  >
                    <img
                      src={require("../../../../res/img/leftbar_grop_icons/invites.svg")}
                      width={25}
                    />
                  </Button>
                ) : this.props.isMinimized &&
                  this.props.label == "Low priority" ? (
                  <Button
                    className="mx_RoomSublist_Minimum_Icon mx_Recommended_Icon"
                    title={this.props.label}
                    onClick={null}
                  >
                    <img
                      src={require("../../../../res/img/leftbar_grop_icons/low_priority.svg")}
                      width={25}
                    />
                  </Button>
                ) : (
  ...
  public render(): React.ReactElement {
    ...
    if (this.props.isSignout) {
      ...
      const resizeBoxClasses = classNames({
        mx_RoomSublist_resizeBox: true,
        mx_Recommend_resizeBox: this.props.tagId == DefaultTagID.Recommends,
      });
      content = (
        <React.Fragment>
          <Resizable
      ...
      const minHeightPadding =
        RESIZE_HANDLE_HEIGHT + (showMoreAtMinHeight ? SHOW_N_BUTTON_HEIGHT : 0);
      const minTilesPx = layout.tilesToPixelsWithPadding(
        minTiles,
        minHeightPadding
      );
      const maxTilesPx = layout.tilesToPixelsWithPadding(
        this.numTiles,
        this.padding
      );
      ...
      if (maxTilesPx > this.state.height) {
        ...
        const nonPaddedHeight =
          this.state.height - RESIZE_HANDLE_HEIGHT - SHOW_N_BUTTON_HEIGHT;
        const amountFullyShown = Math.floor(
          nonPaddedHeight / this.layout.tileHeight
        );
        const numMissing = this.numTiles - amountFullyShown;
        const label = _t("Show %(count)s more", { count: numMissing });
        let showMoreText = (
          <span className="mx_RoomSublist_showNButtonText">{label}</span>
        );
      ...
      if (
        layout.visibleTiles >= this.numTiles &&
        this.numTiles <= layout.minVisibleTiles
      ) {
    ...
    return (
      ...
      >
        {this.renderHeader(this.props.label)}

        {!this.state.isShowRecommendsRoomTiles &&
        this.props.tagId == DefaultTagID.Recommends
          ? false
          : content}
      </div>
    );
