...
import MessageHoverActionBar from "../messages/MessageHoverActionBar";
import ReactionsRow from '../messages/ReactionsRow';
import { getEventDisplayInfo } from '../../../utils/EventUtils';
import GiftsRow from '../messages/GiftsRow';
import { debounce } from "lodash";
import NftAvatarCheckContainer from '../../structures/NftAvatarCheckContainer';

const img = require("../../../../res/img/bots/leon1.png");
...
const Bots = [
    {
        type: "price_bot", name: "Satoshi"
    },
    {
        type: "verification_bot", name: "Hooman"
    }
]
...
interface Gifts {
    pointsGifts: Relations | null,
    cryptoGifts: Relations | null
}
...
interface IProps {
    ...
    memberAvatar?: ReactNode;

    setSentTipAmount?: (value: number) => void;

    disableHover?: boolean;

    setIsShowConfirmation?: any;
...
interface IState {
    ...
    isShowMessageActionBar: boolean;
...
export default class EventTile extends React.Component<IProps, IState> {
    ...
    constructor(props, context) {
        ...
        this.setIsShowMessageActionBar = this.setIsShowMessageActionBar.bind(this);

        this.showMessageActionBar = this.showMessageActionBar.bind(this);
    }
    ...
    componentDidMount() {
        ...
        if (this.props.showReactions) {
            this.props.mxEvent.on("Event.relationsCreated", this.onReactionsOrGiftsCreated);
        }
    ...
    componentWillUnmount() {
        ...
        if (this.props.showReactions) {
            this.props.mxEvent.removeListener("Event.relationsCreated", this.onReactionsOrGiftsCreated);
        }
    }
    ...
    public makeMemberTiles(members: Array<RoomMember | MatrixEvent>) {
        return members.map((m) => {  
            if (m instanceof RoomMember) {
                return <MemberTile key={m.userId} member={m} ref={m.userId} showPresence={true} />;
            }
        });
    }

    public getChildrenJoined = (start: number, end: number, allMembers: Array<RoomMember>): Array<JSX.Element> => {
        return this.makeMemberTiles(allMembers.slice(start, end));
    };
    ...
    getGifts = () => {
        ...
        const eventId = this.props.mxEvent.getId();
        const pointsGifts = this.props.getRelationsForEvent(eventId, "m.present.points", "m.gift");
        const cryptoGifts = this.props.getRelationsForEvent(eventId, "m.present.crypto", "m.gift.crypto");
        const gifts = {
            pointsGifts,
            cryptoGifts
        }
    ...
    private onReactionsOrGiftsCreated = (relationType, eventType) => {
        console.log("=====relationType, eventType=====", relationType, eventType);
        if ((relationType === "m.annotation" && eventType === "m.reaction") || (relationType === "m.present.points" && eventType === "m.gift") || (relationType === "m.present.crypto" && eventType === "m.gift.crypto")) {
            // this.props.mxEvent.removeListener("Event.relationsCreated", this.onReactionsOrGiftsCreated);
            this.setState({
                reactions: this.getReactions(),
                gifts: this.getGifts(),
    ...
    private showMessageActionBarEvent = debounce(() => {
        this.setIsShowMessageActionBar(true);
    }, 200)

    showMessageActionBar = (e) => {
        e.preventDefault();
        let mousePos = {
        ...
        this.showMessageActionBarEvent();
    }

    setIsShowMessageActionBar = (value: boolean) => {
        if(this.state.isShowMessageActionBar !== value) {
            this.setState({isShowMessageActionBar: value});
        }
    }
    ...
    render() {
        ...
        let botName = "";
        Bots.map((bot) => {
            if(bot.type == this.props.mxEvent.getSender() || bot.type == this.props.mxEvent.sender?.rawDisplayName || bot.type == this.props.mxEvent.event.content.botType) {
                botName = bot.name;
                isBot = true;                
            }
        })
        const isMyEvent = (this.props.mxEvent.event.sender === MatrixClientPeg.get().getUserId())
        ...
        const classes = classNames({
            ...
            // mx_EventTile_highlight: this.state.hover || (this.props.tileShape === TileShape.Notif ? false : this.shouldHighlight()),
            ...
            mx_EventTile_My_Event: isMyEvent,
            mx_PriceBot: !!isBot,
        });
        ...
        if(isBot) {
            avatar = 
            (
                <div className={`mx_EventTile_avatar img-size${avatarSize}`}>
                    <div>
                        <div className="mx_AccessibleButton mx_EntityTile mx_EntityTile_online">
                            <div className="mx_EntityTile_avatar">
                                <img src={img} className="mx_BaseAvatar mx_BaseAvatar_image"/>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        else if (this.props.mxEvent.sender && avatarSize) {
            let memberId;
            let member;
            ...
            if (this.props.mxEvent.getContent().third_party_invite) {
                memberId = this.props.mxEvent.target.userId;
                member = this.props.mxEvent.target;
            } else {
                memberId = this.props.mxEvent.sender.userId;
            ...
            try {
                ...
                const avatars = this.getChildrenJoined(0, allMembers.length, allMembers);
                    ...
                    if(member.userId == memberId) {
                        avatar =  (
                            <div className={`mx_EventTile_avatar img-size${avatarSize}`}>
                                ...
                                (
                                    <NftAvatarCheckContainer userId={memberId}>
                                        {avatars[index]}
                ...
                (
                    <div className={`mx_EventTile_avatar img-size${avatarSize}`}>
                        ...
                        (
                            <NftAvatarCheckContainer userId={memberId}>
                                <MemberAvatar member={member} width={avatarSize} height={avatarSize}/>
                            </NftAvatarCheckContainer>
                        )
                        :
                        (
        ...
        if (needsSenderProfile && this.props.hideSender !== true) {
            if (!this.props.tileShape) {
                ...
                sender = <SenderProfile mxEvent={this.props.mxEvent} enableFlair={this.props.enableFlair} isPreview={this.props.isPreview}/>;
            }
        }

        const actionBar = !isEditing && !this.props.disableHover && this.props.mxEvent.getType() ==="m.room.message" ? <MessageActionBar
            mxEvent={this.props.mxEvent}
            ...
            isShowMessageActionBar={this.state.isShowMessageActionBar}
            setIsShowMessageActionBar={this.setIsShowMessageActionBar}
            mousePos={this.state.mousePos}
            setIsShowConfirmation={this.props.setIsShowConfirmation}
        /> : undefined;

        const hoverActionBar = !isEditing && !this.props.disableHover && this.props.mxEvent.getType() ==="m.room.message" ? <MessageHoverActionBar
            mxEvent={this.props.mxEvent}
            reactions={this.state.reactions}
            permalinkCreator={this.props.permalinkCreator}
            getTile={this.getTile}
                ...
                React.createElement(this.props.as || "li", {
                    ...
                    <div className="mx_EventTile_line" key="mx_EventTile_line">
                        ...
                        <span className="mx_MTextBody mx_EventTile_content">
                            <div className='mx_MTextBody_container'>
                                <div className="mx_SenderProfile">
                                    <span className="mx_SenderProfile_displayName mx_Username_color7">
                                        {botName}
                                    </span>
                                </div>
                                <div className="mx_EventTile_body">
                                    {
                                        parse(this.props.mxEvent.event.content.body)
                                    } 
                                </div>
                            </div>
                        </span>
        ...
        else switch (this.props.tileShape) {
            case TileShape.Notif: {
                ...
                return React.createElement(this.props.as || "li", {
                    ...
                    <div className="mx_EventTile_senderDetails" key="mx_EventTile_senderDetails">
                        { avatar }
                        <a href={permalink} onClick={this.onPermalinkClicked}>
                            <div className='mx_EventTile_senderAndGift'>
                                { sender }
                            </div>
                            { timestamp }
                        </a>
                    ...
                    React.createElement(this.props.as || "li", {
                        ...
                        <div className="mx_EventTile_line" key="mx_EventTile_line">
                            ...
                            <EventTileType ref={this.tile}
                                ...
                                senderAndGift={senderAndGift}
                                giftsRow={giftsRow}
                                showMessageActionBar={this.showMessageActionBar}
                                actionBar={actionBar}
                            />
                            { keyRequestInfo }
                            {/* { actionBar } */}
