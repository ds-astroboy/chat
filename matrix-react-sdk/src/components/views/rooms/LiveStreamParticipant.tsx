...
export const LiveStreamParticipant = ({
  participant,
  admin,
  creator
  ...
  useEffect(() => {
    ...
    const user_info_key = `LiveStreamParticipant:user_info:${participant.identity}`;
    ...
    if (isLocal && avatar) {
      ...
      if (userInfoItem) {
        try {
          ...
          console.log('LiveStreamParticipant:user_info:error:', err);
          ...
          if (userInfo && userInfo.ts) {
            ...
            if (userInfo.ts > expiryTs) {
              console.log('LiveStreamParticipant:user_info:purge', userInfo);
              window.localStorage.removeItem(user_info_key);
            }
          }
        }
      } else { // Fetch use info and cache in local storage
  ...
  if (participant.identity === creator) {
    return (
      <div key={participant.sid} className="participant ratio ratio-16x9">
        {mainElement}
      </div>      
    )
  } else {
