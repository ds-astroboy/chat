...
import { getEmailAddressByUserName, verifyWeb2Email } from '../../../../apis';
import dis from "../../../../dispatcher/dispatcher";
import { NOTIFICATIONACTIONS } from '../../../../@variables/common';
...
export default class EmailAddresses extends React.Component {
    ...
    async componentDidMount() {
        ...
        if(!auth) return;
        const {success, email} = await getEmailAddressByUserName(auth);
        if(!success) return;
        this.setState({newEmailAddress: email, disabled: true});
    }
    ...
    _onAddClick = async(e) => {
        ...
        const callbackUrl = "http://cafeteria.gg/#/connect-email"
        const result = await verifyWeb2Email(email, callbackUrl);

        if(result) {
            console.log("action: ", NOTIFICATIONACTIONS.SentEmail);
            dis.dispatch({
                action: NOTIFICATIONACTIONS.SentEmail
            })
        }
    ...
    render() {
        ...
        let addButton = (
            <AccessibleButton onClick={this._onAddClick} kind="primary" className='common-btn bg-green btn-hover-purple small-shadow px-4 py-0 mt-4'>
                { _t("Add") }
            </AccessibleButton>
        );
