...
const AvatarSetting = ({ avatarUrl, avatarAltText, avatarName, uploadAvatar, removeAvatar, clickAdjustableButton }) => {
    ...
    if (uploadAvatar) {
        ...
        uploadAvatarBtn = <AccessibleTooltipButton
            onClick={clickAdjustableButton}
            title="Adjustable Avatar"
    ...
    if (avatarUrl && removeAvatar) {
        removeAvatarBtn = <AccessibleButton onClick={removeAvatar} kind="link_sm" className="px-0">
            { _t("Remove") }
        </AccessibleButton>;
    }
...
AvatarSetting.propTypes = {
    ...
    avatarName: PropTypes.string, // name of user/room the avatar belongs to
