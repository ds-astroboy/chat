...
export function isGIF(model: EditorModel) {
    return startsWith(model, '<gif>', false) && endsWith(model, '</gif>', false);
}
...
export function endsWith(model: EditorModel, prefix: string, caseSensitive = true) {
    const firstPart = model.parts[0];
    // part type will be "plain" while editing,
    // and "command" while composing a message.
    let text = firstPart && firstPart.text;
    ...
    return firstPart && (firstPart.type === "plain" || firstPart.type === "command") && text.endsWith(prefix);
}
