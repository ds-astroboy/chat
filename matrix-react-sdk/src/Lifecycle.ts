...
export function attemptTokenLogin(
    ...
    return sendLoginRequest(
        ...
        return clearStorage(true).then(async () => {
...
export async function onLoggedOut(): Promise<void> {
    ...
    stopMatrixClient();
    await clearStorage(false, { deleteEverything: true });
 ...
 */
async function clearStorage(isnotLocalStorageClear?: boolean, opts?: { deleteEverything?: boolean }): Promise<void> {
    Analytics.disable();

    console.log("isnotLocalStorageClear: ", isnotLocalStorageClear);

    if (!isnotLocalStorageClear && window.localStorage) {
        ...
        if(latesModalNumber) {
            window.localStorage.setItem("latest_modal_number", latesModalNumber);
        }
