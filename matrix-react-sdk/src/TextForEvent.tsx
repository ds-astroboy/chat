...
import { checkIsDisplayName } from './hooks/commonFuncs';
...
function textForMemberEvent(ev: MatrixEvent, allowJSX: boolean, showHiddenEvents?: boolean): () => string | null {
    ...
    const isDisplayName = checkIsDisplayName(targetName);
    if(!isDisplayName) {
        const user = MatrixClientPeg.get().getUser(targetName);
        targetName = user? user.displayName : targetName;
    }
    ...
    switch (content.membership) {
        ...
        case 'join':
            if (prevContent && prevContent.membership === 'join') {
                ...
                //     });
                // } else 
                // if (prevContent.avatar_url && !content.avatar_url) {
                ...
                return () => _t('%(targetName)s joined the group', {targetName });
            }
        // case 'leave':
