...
import { setBarrierToRoom, setCategoryToRoom } from "./apis";
...
export default function createRoom(opts: IOpts, room_uri?: any, barrierObj?: any, category?: string): Promise<string | null> {
    opts = opts || {};
    ...
    return client.createRoom(createOpts).finally(function() {
        ...
        roomId = res.room_id;
        let roomCreatorId = client.credentials.userId;
        client.createRoomDashboardApi(roomCreatorId,roomId).then(function(result){
            console.log("room creation points added successfully", result)
        }).catch(function(e){
            console.log("error occured while calling dashboard points api",e)
        });

        if(room_uri) {
            await client.sendStateEvent(roomId, 'm.room.avatar', { url: room_uri }, '');
        }

        const accessToken = await getAccessToken();

        if(category) {
            setCategoryToRoom(accessToken, category, roomId);
        }

        if(barrierObj) {
            setBarrierToRoom(accessToken, roomId, barrierObj)
        }
