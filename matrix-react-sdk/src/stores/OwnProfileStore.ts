...
const KEY_DISPLAY_WALLETADDRESS = "mx_profile_walletaddress";
...
export class OwnProfileStore extends AsyncStoreWithClient<IState> {
    ...
    private constructor() {
        ...
        super(defaultDispatcher, {
            ...
            walletAddress: window.localStorage.getItem(KEY_DISPLAY_WALLETADDRESS),
        });
    }
    ...
    /**
     * Gets the wallet address for the user, or null if not present.
     */
    public get walletAddress(): string {
        return this.state.walletAddress || null;
    }
    ...
    protected async onNotReady() {
        if (this.monitoredUser) {
            ...
            this.monitoredUser.removeListener("User.walletAddress", this.onProfileUpdate);
        }
    ...
    protected async onReady() {
        ...
        if (this.monitoredUser) {
            ...
            this.monitoredUser.on("User.walletAddress", this.onProfileUpdate);
        }
    ...
    private onProfileUpdate = async () => {
        ...
        const walletAddress = await getWalletAddress(this.matrixClient.getUserId());
        window.localStorage.setItem(KEY_DISPLAY_WALLETADDRESS, walletAddress);
        await this.updateState({ walletAddress: walletAddress });
    };
