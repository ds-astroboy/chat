...
class RoomViewStore extends Store<ActionPayload> {
    ...
    __onDispatch(payload) { // eslint-disable-line @typescript-eslint/naming-convention
        switch (payload.action) {
            ...
            case 'open_bots_settings': {
                // FIXME: Using an import will result in test failures
                const RoomBotsSettingsDialog = sdk.getComponent("dialogs.RoomBotsSettingsDialog");
                Modal.createTrackedDialog('Bots & Plugins', '', RoomBotsSettingsDialog, {
                    roomId: payload.room_id || this.state.roomId,
                    initialTabId: payload.initial_tab_id,
                }, /*className=*/null, /*isPriority=*/false, /*isStatic=*/true);
                break;
    ...
    private async joinRoom(payload: ActionPayload) {
        ...
        const address = this.state.roomAlias || this.state.roomId || payload.roomId;
        ...
        try {
            ...
            CountlyAnalytics.instance.trackRoomJoin(startTime, this.state.roomId || payload.roomId, payload._type);
            ...
            dis.dispatch({
                ...
                roomId: this.state.roomId || payload.roomId,
            });
            const joinedUserId = cli.getUserId();
            cli.joinRoomDashboardApi(joinedUserId,this.state.roomId || payload.roomId).then(function(result){
                console.log("room user gain points added successfully", result)
            }).catch(function(e){
