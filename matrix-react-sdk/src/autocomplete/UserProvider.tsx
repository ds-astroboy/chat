...
import { getUsersByRoomId } from '../apis';
...
export default class UserProvider extends AutocompleteProvider {
    ...
    constructor(room: Room) {
        ...
        this.matcher = new QueryMatcher([], {
            ...
            // funcs: [obj => `@${obj.userId}`], // index by user id minus the leading '@'
    ...
    async getCompletions(
        ...
        if (!this.users) await this.makeUsers();
        ...
        if (fullMatch && fullMatch !== '@') {
            ...
            completions = this.matcher.match(query, limit).map((user) => {
                ...
                return {
                    ...
                    //         { user.matrixUser && <MemberAvatar member={user.matrixUser} width={24} height={24} /> }
                    //     </PillCompletion>
                    // ),
                    component: (
                            <PillCompletion title={user.rawDisplayName}>
    ...
    private async makeUsers() {
        ...
        // const userData = window.localStorage.getItem("mx_userData");
        // if(!userData) return;
        // const auth = JSON.parse(userData);
        // let {success, users} = await getUsersByRoomId(this.room.roomId, auth);
        // if(!success) return;
        // const members = this.room.getJoinedMembers();
        // users.forEach((user) => {
        //     const matrixUser = members.find((member) => {
        //         const matrixId = getUserNameFromId(member.userId);
        //         return (user.username === matrixId);
        //     })
        //     user.matrixUser = matrixUser;
        //     user.searchName = matrixUser.rawDisplayName || user.displayName
        // })
        // this.users = users;
        // this.matcher.setObjects(this.users);
    }
