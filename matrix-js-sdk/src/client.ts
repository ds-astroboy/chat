...
export class MatrixClient extends EventEmitter {
    ...
    public async getUserWalletAddress(userId: string): Promise<Record<string, any>> {
        const path = utils.encodeUri("/account/3pid", {
            $userId: userId,
            $type: "m.account.walletaddress",
        });
        try {
            return await this.http.authedRequest(
                undefined, "GET", path, undefined,
    ...
    /**
     * Join a room. If you have already joined the group, this will no-op.
     ...
     */
     public sendEvent(
        ...
        return this.sendCompleteEvent(roomId, { type: eventType, content }, txnId, callback, isBot);
    }
     ...
     */
     private sendCompleteEvent(
        ...
        const localEvent = new MatrixEvent(Object.assign(eventObject, {
            ...
            user_id: isBot?eventObject.content.botType : this.credentials.userId,
            sender: isBot?eventObject.content.botType : this.credentials.userId,
            room_id: roomId,
    ...
    public sendMessage(
        ...
        if(isBot) {
            return this.sendEvent(roomId, EventType.RoomMessage, content, txnId, callback, true);
        }
        else {
     ...
     * @return {module:http-api.MatrixError} Rejects: with an error response.
     */
     public async setWalletAddress(walletaddress: string, callback?: Callback): Promise<{}> {
        const prom = this.setAccountData("m.account.walletaddress", {walletaddress: walletaddress}, callback);

        // XXX: synthesise a profile update for ourselves because Synapse is broken and won't
        const user = this.getUser(this.getUserId());
        if (user) {
            user.walletAddress = walletaddress;
            user.emit("User.walletAddress", user.events.presence, user);
        }
        return prom;
    }
     ...
     * @return {module:http-api.MatrixError} Rejects: with an error response.
     */
    public signUpDashboardApi(userId: string): Promise<any> {
        const path = utils.encodeUri("/dashboard/api/signup_points/", {});
        return this.http.authedRequest(
            undefined, "POST", path, undefined, { user_id: userId, dashboard_api: true }
        );
    }
     ...
     * @return {module:http-api.MatrixError} Rejects: with an error response.
     */
     public createRoomDashboardApi(userId: string, roomId: string): Promise<any> {
        const path = utils.encodeUri("/dashboard/api/create_room_points/", {});
        return this.http.authedRequest(
            undefined, "POST", path, undefined, { user_id: userId, room_id: roomId, dashboard_api: true }
        );
    }

    /**
     * @param {string} userId
     * @param {string} currentTimeInMs
     * @return {Promise} Resolves: Empty object
     * @return {module:http-api.MatrixError} Rejects: with an error response.
     */
    public loginDashboardApi(userId: string, currentTimeInMs: number): Promise<any> {
        const path = utils.encodeUri("/dashboard/api/login_points/", {});
        return this.http.authedRequest(
            undefined, "POST", path, undefined, { user_id: userId, current_time_in_ms: currentTimeInMs, dashboard_api: true }
        );
    }
     ...
     * @return {module:http-api.MatrixError} Rejects: with an error response.
     */
    public joinRoomDashboardApi(userId: string, roomId: string): Promise<any> {
        const path = utils.encodeUri("/dashboard/api/room_user_join_points/", {});
        return this.http.authedRequest(
            undefined, "POST", path, undefined, {  user_id: userId, room_id: roomId,dashboard_api: true }
        );
    }
     ...
     * @return {module:http-api.MatrixError} Rejects: with an error response.
     */
    public walletConnectDashboardApi(userId: string): Promise<any> {
        const path = utils.encodeUri("/dashboard/api/wallet_connect_points/", {});
        return this.http.authedRequest(
            undefined, "POST", path, undefined, { user_id: userId, dashboard_api: true }
        );
    }
